...
unimplemented-codes:
    @node -e "const fs = require('fs'); const data = JSON.parse(fs.readFileSync('assets/spec.json', 'utf-8')); const codes = data.filter(x => x.unimplemented).map(x => '\`' + x.code + '\`').join(', '); console.log(codes);"
